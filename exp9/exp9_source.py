...
# any-direction-push.
#
# **This experiment (exp9) addresses exactly that.** It keeps
...
#
# 4. **Four steering arms per cell.** For every (domain, strength)
#    cell we run FOUR conditions:
#
#    - **`"targeted"`**     — boost the domain's top-8 purest slots.
#    - **`"matched"`**      — boost 8 purity-matched slots from OTHER domains.
#    - **`"random_dir"`**   — bypasses registry entirely, injects random unit vector
#                             scaled to the SAME norm that 'targeted' injected in
#                             that same cell (different random draw per repeat).
#    - **`"shuffled_dec"`** — same decoder columns, permuted slot-to-column
#                             mapping (fixed seed, logged), bias on same target slot ids.
#
# 5. **REPEATS = 3 per (domain, arm, strength) cell.** Each
#    repeat uses a different generation seed (`torch.manual_seed(1000
#    + rep)`). Total = 4 domains × 4 arms × 6 strengths × 3
#    repeats = **288 runs**. Per cell we report `mean ± std` for
...
#    strength** satisfying four conditions simultaneously:
#
#    (a) targeted hit% mean exceeds its own strength=0 baseline mean
#        by **more than 2× pooled std**,
#    (b) targeted hit% mean exceeds the matched control hit% mean at the
#        **same strength** by **more than 2× pooled std**,
#    (c) the cell is not collapsed in the majority of repeats, AND
#    (d) delta_ratio match between targeted and matched within 35%.
#
...
#
# **Outputs**
# - `exp9_model.pt` — registry state_dict (NOT Pythia).
# - `exp9_summary.json` — purity verdict + training summary (same
#   schema as exp4).
# - `exp9_steering.json` — score-scale stats, derived strengths,
#   fixed gain, the full 288-row per-(domain, arm, strength,
#   repeat) table (hit%, native%, rep%, entropy, delta_ratio,
#   collapsed), per-cell aggregates (`mean ± std`), targeted and
#   control slot ids + purities, dose-match check results, cfg.
# - `exp9_results.png` — per-domain: 4-arm comparison curves
#   with error bars, x = delta_ratio.
# - `exp9_delta_ratio.png` — delta_ratio vs strength, all 4 arms.
#
# ---
...
#
# Expected runtime: ~40-45 minutes total. Training is identical to
# exp3b / exp4b (~19 min). The steering block is 288 runs (4 domains
# x 4 arms x 6 strengths x 3 repeats) — ~20-22 minutes on a T4.
...
torch.save(registry.state_dict(), f"{OUT}/exp9_model.pt")
print(f"saved registry state_dict to {OUT}/exp9_model.pt "
...
plt.tight_layout()
plt.savefig(f"{OUT}/exp9_results.png", dpi=130)
plt.show()
print(f"saved {OUT}/exp9_results.png")
...
with open(f"{OUT}/exp9_summary.json", "w", encoding="utf-8") as f:
    json.dump(summary, f, indent=2, ensure_ascii=False, default=str)
print(json.dumps(summary, indent=2, ensure_ascii=False, default=str))
print(f"\nsaved {OUT}/exp9_summary.json")
...
# %%
class _SteeringHook:
    """Forward hook installed on Pythia's layer-12 block. Supports four arms:
    1. 'targeted': registry patch on top domain slots
       delta = decode(topk(pre + steer_bias)) - decode(topk(pre))
       h_steered = h + GAIN * delta
    2. 'matched': same registry patch on purity-matched non-domain slots
    3. 'random_dir': bypasses registry entirely, injecting random unit vector
       scaled to match the targeted arm's injected delta norm:
       delta_random = r * (gain * delta_targeted).norm()
       h_steered = h + delta_random
    4. 'shuffled_dec': same decoder columns, permuted slot-to-column mapping:
       W_shuf = registry.decoder.weight[:, perm]
       delta = decode_with(W_shuf, topk(pre+bias)) - decode_with(W_shuf, topk(pre))
       h_steered = h + GAIN * delta

    Accumulates delta_ratio diagnostic for all arms:
        delta_ratio = injected_delta.norm(dim=-1).mean() / h.norm(dim=-1).mean()
    When strength == 0.0 (no bias / zero delta), delta_ratio is exactly 0.0."""

    def __init__(self, registry):
        self.registry = registry
        self.gain = 1.0
        self.mode = "standard"  # "standard", "random_dir", "shuffled_dec"
        self.random_delta = None
        self.shuf_weight = None
        self._dr_sum = 0.0
        self._dr_count = 0
        self._norm_sum = 0.0
    ...
    def set_mode(self, mode="standard", random_delta=None, shuf_weight=None):
        assert mode in ("standard", "random_dir", "shuffled_dec")
        self.mode = mode
        self.random_delta = random_delta
        self.shuf_weight = shuf_weight
    ...
    @property
    def injected_norm(self):
        return self._norm_sum / self._dr_count if self._dr_count > 0 else 0.0
    ...
    def _patch_delta(self, h):
        ...
        if self.mode == "shuffled_dec":
            dec_base  = F.linear(acts_base,  self.shuf_weight.to(dtype=acts_base.dtype, device=acts_base.device))
            dec_steer = F.linear(acts_steer, self.shuf_weight.to(dtype=acts_steer.dtype, device=acts_steer.device))
        else:
    ...
    def __call__(self, module, inputs, output):
        ...
        if self.mode == "random_dir":
            if self.random_delta is not None:
                delta = self.random_delta.to(device=h.device, dtype=h.dtype)
                steered = h + delta
                inj_norm = float(delta.norm(dim=-1).mean().item())
                if inj_norm > 0.0:
                    denom = h.norm(dim=-1).mean().item()
                    if denom > 0.0:
                        self._dr_sum += inj_norm / denom
                        self._dr_count += 1
                        self._norm_sum += inj_norm
            else:
                steered = h
        else:
            ...
            if float(reg.steer_bias.abs().sum().item()) != 0.0:
                denom = h.norm(dim=-1).mean().item()
                if denom > 0.0:
                    inj_norm = (self.gain * delta).norm(dim=-1).mean().item()
                    self._dr_sum += inj_norm / denom
                    self._dr_count += 1
                    self._norm_sum += inj_norm
...
# exp9: BOOST strengths anchored to theta (the average per-token k-th
...
ARM_NAMES = ("targeted", "control", "random_dir", "shuffled_dec")
print(f"  total runs          = {len(DOMAIN_NAMES)} domains * "
      f"{len(ARM_NAMES)} arms * {len(BOOST_STRENGTHS)} strengths * {REPEATS} repeats "
      f"= {len(DOMAIN_NAMES) * len(ARM_NAMES) * len(BOOST_STRENGTHS) * REPEATS}")
...
raw_runs = {d: {arm: [] for arm in ARM_NAMES} for d in DOMAIN_NAMES}

print("=" * 78)
print(f"exp9 steering sweep  (GAIN={GAIN}, REPEATS={REPEATS}, "
...
# Shuffled-decoder weight (one fixed permutation, built once before the
# sweep) + random-direction seed base. W_shuf keeps the SAME decoder
# columns, only permuted slot-to-column mapping:
#     W_shuf = decoder.weight[:, perm]
SHUF_SEED = 777
RANDOM_DIR_SEED_BASE = 50000
_shuf_gen = torch.Generator(device="cpu").manual_seed(SHUF_SEED)
_shuf_perm = torch.randperm(cfg.n_slots, generator=_shuf_gen)
W_shuf = registry.decoder.weight[:, _shuf_perm].detach().clone().to(DEVICE)
print(f"shuffled_dec: built W_shuf from one fixed permutation "
      f"(seed={SHUF_SEED}, n_slots={cfg.n_slots})")
...
for d in DOMAIN_NAMES:
    ...
    if (not target_slots[d]) or (not control_slots[d]):
        ...
        continue
    d_idx = DOMAIN_NAMES.index(d)
    targeted_norms = {}  # (strength_float, rep_n) -> injected norm from targeted arm
    for condition in ARM_NAMES:
        # slot ids per arm: targeted + shuffled_dec share the target ids;
        # control uses the purity-matched ids; random_dir uses no slots.
        if condition == "targeted":
            slot_ids = target_slots[d]
        elif condition == "control":
            slot_ids = control_slots[d]
        elif condition == "shuffled_dec":
            slot_ids = target_slots[d]
        else:  # "random_dir"
            slot_ids = []
        for s_idx, strength in enumerate(BOOST_STRENGTHS):
            for rep_n in range(1, REPEATS + 1):
                ...
                _steering_hook.set_gain(GAIN)
                if condition in ("targeted", "control"):
                    bias = _make_bias(slot_ids, strength)
                    _steering_hook.set_mode("standard")
                    try:
                    ...
                    bias = _make_bias(slot_ids, strength)
                    _steering_hook.set_mode("shuffled_dec", shuf_weight=W_shuf)
                    try:
                        with registry.steering(bias):
                            toks, ents = generate([EOT], n_new=GEN_LEN)
                    finally:
                        _steering_hook.set_mode("standard")
                else:  # "random_dir": dose-matched to targeted's injected norm
                    # in the SAME cell (same domain, strength, rep). ARM_NAMES
                    # orders random_dir after targeted, so the entry exists.
                    tgt_norm = float(targeted_norms.get((float(strength), rep_n), 0.0))
                    if tgt_norm > 0.0:
                        g = torch.Generator(device="cpu").manual_seed(
                            RANDOM_DIR_SEED_BASE + d_idx * 10000 + s_idx * 100 + rep_n)
                        r = torch.randn(cfg.hidden_size, generator=g)
                        r = r / (r.norm() + 1e-12) * tgt_norm
                        random_delta = r.to(dtype=torch.float32)
                    else:
                        random_delta = torch.zeros(cfg.hidden_size, dtype=torch.float32)
                    bias = _make_bias([], 0.0)  # no registry bias; dose via random_delta
                    _steering_hook.set_mode("random_dir", random_delta=random_delta)
                    try:
                        with registry.steering(bias):
                            toks, ents = generate([EOT], n_new=GEN_LEN)
                    finally:
                ...
                _steering_hook.set_mode("standard")  # no state leaks to next run
                if condition == "targeted":
                    targeted_norms[(float(strength), rep_n)] = float(_steering_hook.injected_norm)
...
cell_results = {d: {c: {} for c in ARM_NAMES} for d in DOMAIN_NAMES}
for d in DOMAIN_NAMES:
    for c in ARM_NAMES:
...
# Summary tables: one row per (domain, strength), one column per arm.
# First table = hit% mean, second = native% mean. Values use the
# per-cell means from cell_results (recorded for every arm on every row).
print("=" * 100)
print(f"EXP9 SUMMARY -- hit% mean per (domain, strength) per arm")
print("=" * 100)
_hdr = f"{'domain':>10s} | {'strength':>10s} | " + " | ".join(f"{a:>12s}" for a in ARM_NAMES)
print(_hdr)
print("-" * len(_hdr))
for d in DOMAIN_NAMES:
    for s in BOOST_STRENGTHS:
        _vals = []
        for a in ARM_NAMES:
            _cell = cell_results[d][a].get(s, None)
            _vals.append(_cell["hit_mean"] * 100 if (_cell is not None and _cell["n"] > 0) else float("nan"))
        print(f"{d:>10s} | {s:>10.4f} | " + " | ".join(f"{v:12.1f}" for v in _vals))
print("=" * len(_hdr))
print(f"EXP9 SUMMARY -- native% mean per (domain, strength) per arm")
print("=" * 100)
print(_hdr)
print("-" * len(_hdr))
for d in DOMAIN_NAMES:
    for s in BOOST_STRENGTHS:
        ...
        for a in ARM_NAMES:
            _cell = cell_results[d][a].get(s, None)
            _vals.append(_cell["native_mean"] * 100 if (_cell is not None and _cell["n"] > 0) else float("nan"))
        print(f"{d:>10s} | {s:>10.4f} | " + " | ".join(f"{v:12.1f}" for v in _vals))
print("=" * len(_hdr))

# Dose-match check: within each (domain, strength) cell the four arms'
# delta_ratio means must agree within 5% of the largest. Baseline cells
# where all four are ~0 pass trivially. Print any cell that fails.
print("EXP9 DOSE-MATCH CHECK -- four-arm delta_ratio must agree within 5% per cell")
_dose_fail = 0
for d in DOMAIN_NAMES:
    for s in BOOST_STRENGTHS:
        _drs = {}
        ...
        for a in ARM_NAMES:
            _cell = cell_results[d][a].get(s, None)
            if _cell is None or _cell["n"] == 0:
                _ok = False
                break
            _drs[a] = float(_cell["dr_mean"])
        if not _ok:
            print(f"  {d} strength={s:.4f}: MISSING repeats for some arm (drs={_drs})")
            _dose_fail += 1
            continue
        _mx = max(_drs.values())
        _mn = min(_drs.values())
        _gap = (abs(_mx - _mn) / max(_mx, 1e-9)) if _mx > 0 else 0.0
        if _gap > 0.05:
            _detail = ", ".join(f"{a}={_drs[a]*100:.3f}%" for a in ARM_NAMES)
            print(f"  FAIL {d} strength={s:.4f}: gap={_gap*100:.1f}%  {_detail}")
            _dose_fail += 1
if _dose_fail == 0:
    print("  dose-match: all cells agree within 5%")
else:
    print(f"  dose-match: {_dose_fail} cell(s) FAILED the 5% check")
...
# Two figures:
#
# 1. `exp9_results.png` -- one row of 4 panels (one per domain).
...
#
# 2. `exp9_delta_ratio.png` -- delta_ratio vs requested strength,
...
plt.suptitle(f"exp9 steering: targeted (solid) vs control (dashed) ±1 std, "
...
plt.suptitle(f"exp9 delta_ratio diagnostic -- targeted vs control (lines should overlap)")
plt.tight_layout(rect=[0, 0, 1, 0.95])
plt.savefig(f"{OUT}/exp9_delta_ratio.png", dpi=130)
plt.show()
print(f"saved {OUT}/exp9_delta_ratio.png")


# %% [markdown]
# ## 18. Steering JSON
#
# `exp9_steering.json`: score-scale stats, derived strengths, the
...
# `exp9_summary.json` (saved earlier, section 8) carries the
...
# all individual repeats (288 = 4*4*6*3)
for d in DOMAIN_NAMES:
    for c in ARM_NAMES:
...
with open(f"{OUT}/exp9_steering.json", "w", encoding="utf-8") as f:
    json.dump(steering_payload, f, indent=2, ensure_ascii=False, default=str)

print(f"saved {OUT}/exp9_steering.json "
      f"({len(steering_payload['runs'])} individual repeats + "
      f"{sum(len(steering_payload['cells'].get(d, {}).get(c, {})) for d in DOMAIN_NAMES for c in ARM_NAMES)} cells, "
      f"{len(DOMAIN_NAMES)} domains, "
      f"{len(ARM_NAMES)} conditions, {len(BOOST_STRENGTHS)} strengths, "
      f"{REPEATS} repeats)")
