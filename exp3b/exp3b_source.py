...
#
# **This experiment:** Load `EleutherAI/pythia-410m`, freeze it entirely, and
# stream its layer-12 residual stream into a 4096-slot TopK dictionary trained
...
# - Pythia-410m, **fully frozen** (`requires_grad=False` on every parameter,
#   `torch.no_grad()` around every forward call)
# - Activation source: `output_hidden_states=True`, take `hidden_states[12]`
#   (middle of the 24-layer stack), shape `[B, T, 1024]`
# - **Streamed on-the-fly** inside the training loop -- never persisted to
#   disk (4 domains x millions of tokens x 1024 dims would be far too large)
...
#
# Expected runtime: ~35-60 minutes total. Pythia-410m forward is ~3x the
# compute of pythia-160m and only a single T4 is used (not T4x2), so the
# token budget is shrunk to ~3M tokens/domain (~5x less than exp0d's 15M)
# to keep wall-time within the Kaggle single-GPU budget.
#
# ---
...
# setup and the rest of the project needs to be rethought. exp3b is the
# natural next step after exp3 (larger pretrained model, otherwise the
# same frozen-trunk + 4096-slot TopK setup as exp3).
...
torch.save(registry.state_dict(), f"{OUT}/exp3b_model.pt")
print(f"saved registry state_dict to {OUT}/exp3b_model.pt "
...
plt.tight_layout()
plt.savefig(f"{OUT}/exp3b_results.png", dpi=130)
plt.show()
print(f"saved {OUT}/exp3b_results.png")
...
with open(f"{OUT}/exp3b_summary.json", "w", encoding="utf-8") as f:
    json.dump(summary, f, indent=2, ensure_ascii=False, default=str)
print(json.dumps(summary, indent=2, ensure_ascii=False, default=str))
print(f"\nsaved {OUT}/exp3b_summary.json")
