# %% [markdown]
# # Experiment 1d — Annealed supervision (linear ramp to zero by alpha_start_step)
#
# **The single question:** The reserved-range collapse in exp1 and exp1b
# (`final_dead_frac = 44.9%` and `48.2%`, respectively, vs exp1c's clean
# `6.9%` with `lambda_supervision = 0.0`) correlates tightly with `alpha`
# crossing ~0.7–0.8 around step 3400, not with the supervision weight
# itself. Yet exp1 and exp1b both hit `99.6–99.8%` reserved-block
# classification accuracy, while exp1c dropped to `31.3%`. So we have a
# genuine trade-off: a strong, constant supervision signal *does* the
# steering work we want, but it does not prevent (and may even worsen) the
# alpha-driven collapse once the registry is forced to carry information.
# Can we keep the steering benefit while shedding the gradient damage,
# by only applying the supervision signal *before* the alpha ramp begins
# (and therefore before the collapse starts)?
#
# **The mechanism, layered on top of exp1b:**
#
# - 4 domains (unchanged from exp1/exp1b): medicine / law / code / literature
...
# - The reserved-range architecture is kept structurally identical to exp1b:
...
# - What changes is the supervision loss *weight*: instead of a single scalar
#   `cfg.lambda_supervision`, we use a step-dependent schedule
#   `supervision_weight_at(step)`. `cfg.lambda_supervision = 0.2` is now the
#   **peak / starting value** (at step 0). The weight anneals linearly down
#   to `0.0` by `cfg.alpha_start_step` (1500), which is exactly when `alpha`
#   begins ramping from 0 toward 1.0 — i.e. exactly when exp1 and exp1b's
#   res_live / free_live started collapsing. After step 1500 the weight
#   stays at `0.0` for the rest of training. The two schedules
#   (`alpha_at`, `supervision_weight_at`) share `cfg.alpha_start_step` so
#   they stay coupled if that anchor is ever changed.
# - Free-region analysis (`[1024:4096]`) is the same purity-gap test as
#   exp1b/exp1c, just restricted to that subset.
#
# **Why this should help — three observations from the prior runs:**
#
# 1. **The collapse is structurally tied to alpha, not to lambda.** exp1
#    (`lambda=0.2`) and exp1b (`lambda=0.05`) both held res_live / free_live
#    near 95–100% through roughly step 3000, and *both* then entered a
#    relentless, monotonic collapse down to 44.9% / 48.2% dead, starting
#    almost exactly when `alpha` crossed ~0.7–0.8 (~step 3400+) — regardless
#    of the 4x lambda difference.
# 2. **exp1c's zero-supervision run stayed healthy.** With `lambda=0.0`,
#    `final_dead_frac = 6.9%` and `reserved live = 677/1024` — proving the
#    architecture itself does not have to collapse. It just doesn't steer
#    the reserved blocks at all (clf acc = 31.3%).
# 3. **supervision_loss converges fast.** In every run with `lambda > 0`,
#    `supervision_loss` itself drops to near zero within the first ~200–300
#    steps — the classification objective is very easy to learn. It does its
#    useful steering work almost immediately and does not need to remain
#    active for the rest of training.
#
# So: turn supervision **on** for the first 1500 steps (when alpha = 0,
# when slots are 100% healthy, when sup_loss converges fast anyway), then
# turn it **off** exactly when alpha begins to ramp up (which is exactly
# when the collapse begins). The early window should be enough to plant
# the reserved-block alignment — and turning it off should let the
# alpha-driven phase proceed without the supervision gradient adding fuel
# to the dead-slot cascade.
#
...
#
# ---
# **Variant note (exp1d):** exp1d is the annealed-supervision variant — the
# supervision weight linearly decays from `lambda_supervision = 0.2` (the
# same peak as exp1) at step 0 down to `0.0` at `cfg.alpha_start_step`
# (1500), synchronized with the start of the alpha ramp, and stays at 0 for
# the rest of training. The motivation comes from the three Kaggle runs
# already on record: exp1 (`lambda=0.2`, constant) and exp1b
# (`lambda=0.05`, constant) both held healthy `res_live` / `free_live`
# (~95–100%) through step ~3000 and *then* entered a relentless collapse
# down to 44.9% / 48.2% dead — but the collapse's onset correlated with
# alpha crossing ~0.7–0.8 (~step 3400+), not with the lambda value or the
# raw step count. exp1c (`lambda=0.0`) proved that zero supervision keeps
# slots healthy (`dead_frac = 6.9%`) but loses the reserved-block steering
# (clf acc = 31.3%). Finally, `supervision_loss` itself converges to near
# zero within ~200–300 steps in every `lambda > 0` run — so the
# classification objective does its useful work almost immediately and
# does not need to remain active for the full 6000 steps. By turning
# supervision on only during the early "alpha = 0" window (when all slots
# are still 100% healthy) and turning it off before the alpha ramp begins
# (when the collapse starts in exp1 / exp1b), we should get the best of
# both worlds: strong reserved-block classification accuracy close to
# exp1/exp1b's 99%+ AND healthy slot utilization close to exp1c's ~7% or
# exp0d's 3.1% `final_dead_frac`. The reserved-block accuracy check still
# runs at the end — it will tell us whether whatever alignment the early
# supervision established persists through the rest of training
# (interesting result either way). Everything else is unchanged from
# exp1b/exp1c: same four domains, same `n_slots=4096, k=16`,
# `reserved_per_domain=256`, same 6000 steps, same alpha schedule itself
# (unchanged — only the supervision weight schedule is new), same other
# losses, same eval logic, same `res_live` / `free_live` logging.
...
class CFG:
    ...
    # exp1d: lambda_supervision is now the PEAK / starting value of an
...
def supervision_weight_at(step):
    ...
    cfg.alpha_start_step -- supervision does its steering work early while
    all slots are still healthy (alpha=0), and is fully off before the
    alpha ramp begins, which is exactly when exp1/exp1b showed the
    dead-slot collapse starts. Held at 0 for the rest of training.
    """
    if step >= cfg.alpha_start_step:
        return 0.0
    return cfg.lambda_supervision * (1.0 - step / cfg.alpha_start_step)
...
torch.save(model.state_dict(), f"{OUT}/exp1d_model.pt")
...
plt.tight_layout()
plt.savefig(f"{OUT}/exp1d_results.png", dpi=130)
plt.show()
...
with open(f"{OUT}/exp1d_summary.json", "w", encoding="utf-8") as f:
