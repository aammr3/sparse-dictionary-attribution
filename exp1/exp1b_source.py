# %% [markdown]
# # Experiment 1b — Lower supervision weight (0.05) to fix exp1's dead-slot collapse
#
...
#   `lambda_supervision = 0.05` — encouraging, not architecturally forcing.
...
#
# ---
# **Variant note (exp1b):** exp1b is a targeted single-variable follow-up to
# exp1 — identical in every respect except `lambda_supervision` is lowered
# 4x, from 0.2 to 0.05. The motivation comes from exp1's actual Kaggle
# result. The supervision signal did work: reserved-block classification
# accuracy reached 99.6% and the free-region `[1024:4096]` gap over raw
# dims stayed positive at +0.164 (weaker than exp0d's +0.222 but still a
# clear win). The problem was slot utilization: `supervision_loss`
# collapsed to ~0 within the first ~200-300 training steps (108.245 at
# step 1, 1.039 at step 100, 0.029 at step 200, then ~0 with small blips
# for the rest of training), and the whole-registry `final_dead_frac`
# blew up to 44.9% — vs exp0d's 3.1% baseline at the same `n_slots=4096,
# k=16`. The reading is that 0.2 was so much stronger than needed that
# the network found a degenerate low-slot-count solution that satisfied
# the classification objective almost immediately, after which most
# reserved (and by knock-on effect, free-region) slots had no incentive
# to stay active. Dropping the supervision weight 4x should still steer
# the reserved blocks (the classification signal was apparently very
# easy to learn — 99.6% accuracy) while leaving enough gradient
# headroom for AuxK and the LM/reconstruction objectives to keep slot
# diversity healthy. Everything else in the experiment is unchanged
# from exp1: same four domains, same `n_slots=4096, k=16`,
# `reserved_per_domain=256`, same 6000 steps, same alpha schedule,
# same losses, same eval logic.
...
torch.save(model.state_dict(), f"{OUT}/exp1b_model.pt")
...
plt.tight_layout()
plt.savefig(f"{OUT}/exp1b_results.png", dpi=130)
plt.show()
...
with open(f"{OUT}/exp1b_summary.json", "w", encoding="utf-8") as f:
