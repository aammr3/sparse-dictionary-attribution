# %% [markdown]
# # Experiment 1c — Supervision-ablation control (lambda_supervision = 0)
#
# **The single question:** If we reserve named blocks of registry slots for
# specific domains (medicine, law, code, literature) — keeping the *structure*
# of the reserved-range architecture from exp1/exp1b in place — but set the
# supervision loss weight to **zero**, does the registry still collapse to
# ~45–50% dead slots the way exp1 and exp1b did, or does it stay close to
# exp0d's 3.1% baseline? This is the missing single-variable control from
# Experiment 1's original four-run plan.
#
...
#   runs every step (so the per-100-step log still reports `sup`), and the
#   reserved-block classification accuracy check still runs at the end. What
#   changes is purely the loss weight: `lambda_supervision = 0.0`. The combined
#   loss is still `cfg.lambda_supervision * o["supervision_loss"]`, which now
#   contributes nothing to gradients, so this is a clean single-variable test
#   of whether the supervision signal is what was driving the dead-slot
#   collapse in exp1/exp1b.
...
#
# ---
# **Variant note (exp1c):** exp1c is the supervision-ablation control —
# identical to exp1b in every respect except `lambda_supervision` is set to
# `0.0` (full ablation, not just a lower weight). The motivation comes from
# exp1 and exp1b's actual Kaggle results: exp1 (`lambda=0.2`) collapsed to
# `final_dead_frac = 44.9%` and exp1b (`lambda=0.05`) collapsed to
# `final_dead_frac = 48.2%` — *worse*, not better, despite the 4x weight
# reduction. Both runs reserved 256 slots per domain, reached 99.6–99.8%
# reserved-block classification accuracy, and even held `res_live` and
# `free_live` near 95–100% through roughly step 3000 (with mid-training
# recoveries). The new per-100-step `res_live`/`free_live` logging added in
# exp1b revealed that the collapse's onset correlates with `alpha` crossing
# ~0.7–0.8 (around step 3400+), not with `lambda_supervision` or with raw step
# count: both runs then entered a relentless, monotonic collapse down to
# ~45–50% dead by step 6000 regardless of the 4x weight difference. This
# strongly suggests the collapse is *structural* — driven by the 4-domain
# data mixture and/or the alpha curriculum interacting with TopK competition
# dynamics at this registry size — not by the supervision loss's gradient
# magnitude.
#
# This run asks the cleanest possible version of the question: with **zero**
# supervision signal, does the same collapse still happen? If `dead_frac`
# stays close to exp0d's 3.1% baseline, the supervision signal is the cause
# and needs a *structural* fix (e.g. detaching its gradient from the trunk,
# or a different loss formulation) rather than just a smaller weight. If
# the collapse reappears anyway, the cause is structural (4-domain mixture,
# alpha schedule × TopK dynamics at `n_slots=4096, k=16`), and supervision
# is innocent of it. The reserved-block classification accuracy check still
# runs at the end as a useful sanity check on its own: it will measure
# whether the *architecture* alone, with zero training signal tying it to
# domains, produces any accidental domain alignment. Everything else is
# unchanged from exp1b: same four domains, same `n_slots=4096, k=16`,
# `reserved_per_domain=256`, same 6000 steps, same alpha schedule, same
...
torch.save(model.state_dict(), f"{OUT}/exp1c_model.pt")
...
plt.tight_layout()
plt.savefig(f"{OUT}/exp1c_results.png", dpi=130)
plt.show()
...
with open(f"{OUT}/exp1c_summary.json", "w", encoding="utf-8") as f:
