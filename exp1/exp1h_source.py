# %% [markdown]
# # Experiment 1h — Seed-sensitivity check at exp1f's window (anneal_end=1000, SEED 2024)
#
...
#
# ---
# **Variant note (exp1h):** exp1h is a **seed-sensitivity check**, not a
# new hyperparameter point. `anneal_end = 1000` is unchanged from
# exp1f; only `SEED` changed (`1337 -> 2024`). The question: is
# exp1f's `clf_acc = 68.4%` a robust effect of the 1000-step window,
# or an artifact of that one seed? The motivation is exp1g, which
# used the SAME `SEED = 1337` as exp1f at `anneal_end = 650` and
# collapsed to one class (`clf_acc = 23.6%`, 97.9% of predictions =
# "code") — i.e. the response is *not* a smooth / monotonic function
# of `anneal_end` alone, so we cannot yet tell whether 1000's 68.4%
# is a robust effect or a lucky roll. Re-running with `SEED = 2024`
# (the only change vs exp1f) directly tests that. Everything else
# is byte-identical to exp1f: same four domains, same `n_slots=4096,
# k=16`, `reserved_per_domain=256`, same 6000 steps, same alpha
# schedule (`alpha_start_step = 1500` untouched), same `pred_dist`
# diagnostic in `reserved_block_accuracy`, same fixed "balanced=1920"
# print label, same other losses / eval logic / logging.
...
torch.save(model.state_dict(), f"{OUT}/exp1h_model.pt")
...
plt.tight_layout()
plt.savefig(f"{OUT}/exp1h_results.png", dpi=130)
plt.show()
...
with open(f"{OUT}/exp1h_summary.json", "w", encoding="utf-8") as f:
