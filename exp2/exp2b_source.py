# %% [markdown]
# # Experiment 2b — Steering/control experiment, calibrated strengths (Phase F)
#
# **Foundation:** exp2b is a one-delta variant of `exp2/exp2_source.py`.
# Training, model architecture, eval block, domain vocab lists, generation
# harness, collapse detector, verdict logic, and output schema are all
# byte-identical to exp2 — same `n_slots=4096, k=16, reserved_per_domain=256`,
# same four domains (medicine / law / code / literature), same
# `supervision_anneal_end=1000`, same `SEED = 2024`, same 6000 training
# steps, same alpha schedule, same purity diagnostics, same
# `reserved_block` classification check with `pred_dist`. The trained
# model here is a fresh exp1h-equivalent run, not a checkpoint load.
#
# **Why exp2b exists (the only delta vs exp2):** exp2 used fixed
# strength values `[0, 5, 10, 20, 40]` added to the registry's raw
# pre-topk slot scores (`pre`). In two independent training runs that
# produced fluency collapse at the **first nonzero** strength (5) in
# every domain, with no visible gradual transition — strongly suggesting
# those constants were much larger than the natural scale of `pre`,
# so we jumped straight past any informative middle ground. exp2b
# therefore (a) measures the actual scale of `pre` from the freshly
# trained model with a ~10-batch diagnostic (mean, std, and the
# average per-token k-th largest value of `pre` — the actual top-k
# selection threshold) and (b) derives the BOOST and SUPPRESS strength
# lists from the measured std `sigma`, as
# `[0, 0.5*sigma, 1*sigma, 2*sigma, 4*sigma, 8*sigma]`. Both directions
# still apply the bias as before (BOOST positive, SUPPRESS negative);
# the old `strength=10000` extreme in the SUPPRESS list is dropped
# because `8*sigma` should already span the whole informative range
# given the new calibration. Every other piece of the steering
...
#   `[0, 0.5*sigma, 1*sigma, 2*sigma, 4*sigma, 8*sigma]` (sigma =
#   measured pre-stats std), set `steer_bias[d*256:(d+1)*256] =
#   +strength`, generate **200 tokens** greedily from a neutral
#   1-token prompt (just the `<|eot|>` boundary token), and record
#   three metrics: `domain_d_vocab_hit_rate` (fraction of generated
#   tokens inside d's vocab list), `repetition_rate` (fraction of
#   generated bigrams that are repeats of an earlier bigram in the
#   same generation), and `avg_entropy` (mean entropy in **nats** of
#   the softmax next-token distribution over the 200 steps).
# - **SUPPRESS** direction: for each domain `d`, for strength in the
#   same calibrated list `[0, 0.5*sigma, 1*sigma, 2*sigma, 4*sigma,
#   8*sigma]`, set `steer_bias[d*256:(d+1)*256] = -strength`, generate
#   200 tokens greedy from a real **50-token prefix** drawn uniformly
#   from d's eval corpus, and record the same three metrics. We
#   expect `domain_d_vocab_hit_rate` to drop vs the `strength=0`
...
# `avg_entropy < 1.0` as **"fluency broken here"** in the printed
# output. No human judgment required — if boosting collapses to a loop,
# we mark it. This lets us say e.g. "boost code is healthy through
# +2*sigma but collapses at +4*sigma" purely from the metrics.
#
...
# to the `strength=0` baseline. Same three-tier style as the exp1h
# verdict block.
#
# **Outputs:**
# - `exp2b_model.pt` — the freshly trained model.
# - `exp2b_steering_results.json` — every metric per (domain, direction,
#   strength), plus the per-domain vocab lists and the resolved numeric
#   strength values used.
# - `exp2b_results.png` — extends the exp1h-style plot with two extra
...
torch.save(model.state_dict(), f"{OUT}/exp2b_model.pt")
...
BOOST_STRENGTHS     = [0.0, 0.5 * sigma, 1.0 * sigma, 2.0 * sigma, 4.0 * sigma, 8.0 * sigma]
...
for d_idx, d in enumerate(DOMAIN_NAMES):
    ...
    for strength in BOOST_STRENGTHS:
        ...
        print(f"{d:>10s}  {strength:>10.4f}  {hit*100:5.1f}%  {rep*100:5.1f}%  "
...
plt.tight_layout()
plt.savefig(f"{OUT}/exp2b_results.png", dpi=130)
plt.show()
...
with open(f"{OUT}/exp2b_summary.json", "w", encoding="utf-8") as f:
...
# %% [markdown]
# ## 11. Steering JSON dump — `exp2b_steering_results.json`
#
...
with open(f"{OUT}/exp2b_steering_results.json", "w", encoding="utf-8") as f:
    json.dump(steering_payload, f, indent=2, ensure_ascii=False)
print(f"\nwrote {OUT}/exp2b_steering_results.json  "
...
print("\nALL EXP2B OUTPUT FILES:")
print(f"  {OUT}/exp2b_model.pt")
print(f"  {OUT}/exp2b_steering_results.json")
print(f"  {OUT}/exp2b_results.png")
