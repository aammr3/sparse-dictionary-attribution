# %% [markdown]
# # Experiment 2c — Targeted-slot BOOST, theta-calibrated (Phase F)
#
# **Foundation:** exp2c is a one-delta variant of `exp2/exp2b_source.py`.
# Training, model architecture, eval block, domain vocab lists, generation
# harness, collapse detector, and SUPPRESS mechanism / strengths are all
# byte-identical to exp2b — same `n_slots=4096, k=16, reserved_per_domain=256`,
...
#
# **Why exp2b existed (carried over):** exp2 used fixed strength values
...
#
# **Why exp2c exists (the only delta vs exp2b):** exp2b's SUPPRESS
# direction worked cleanly across all 4 domains — the hit-rate on a
# domain's vocabulary dropped sharply near the natural top-k threshold
# — but its BOOST direction failed (0% effect for law / code /
# literature, marginal / noisy for medicine) with fluency collapsing
# at larger strengths. **Hypothesis:** adding a flat positive bias to
# all 256 reserved slots at once floods the model's `k=16` top-k
# budget with an incoherent mix of slots that were never co-active in
# training, wiping the token's whole representation instead of
# steering it. exp2c fixes that by **boosting only the 2
# highest-purity slots per domain** (specific known-good indices
# identified from the per-slot purity analysis already in the script),
# calibrated to `theta = pre_stats["topk_threshold"]` — the actual
# value needed to guarantee top-k inclusion — rather than to the
# block-wide std `sigma`. Concretely:
#
...
#   0.02`).
#
# **Mechanism (one new tensor on the registry, unchanged from exp2b):**
#
...
#   exp2c only changes WHICH entries of `steer_bias` are nonzero in
#   the BOOST direction; the underlying mechanism is unchanged.
#
...
#   greedily from a neutral 1-token prompt (just the `<|eot|>`
#   boundary token), and record three metrics:
...
# - **SUPPRESS** direction (unchanged from exp2b): for each domain `d`,
#   for strength in the same calibrated list `[0, 0.5*sigma, 1*sigma,
#   2*sigma, 4*sigma, 8*sigma]`, set `steer_bias[d*256:(d+1)*256] =
#   -strength` (all 256 reserved slots — whole block, sigma-based),
...
#
# **Automatic collapse-point detector** (unchanged from exp2b). We
...
# Same three-tier style as the exp2b verdict block.
#
# **Outputs:**
# - `exp2c_model.pt` — the freshly trained model.
# - `exp2c_steering_results.json` — every metric per (domain,
#   direction, strength), plus the per-domain vocab lists, the
#   resolved numeric strength values used, and the chosen
#   `target_slots` per domain.
# - `exp2c_results.png` — extends the exp1h-style plot with two extra
...
torch.save(model.state_dict(), f"{OUT}/exp2c_model.pt")
...
plt.tight_layout()
plt.savefig(f"{OUT}/exp2c_results.png", dpi=130)
plt.show()
...
with open(f"{OUT}/exp2c_summary.json", "w", encoding="utf-8") as f:
...
# %% [markdown]
# ## 11. Steering JSON dump — `exp2c_steering_results.json`
#
...
with open(f"{OUT}/exp2c_steering_results.json", "w", encoding="utf-8") as f:
    json.dump(steering_payload, f, indent=2, ensure_ascii=False)
print(f"\nwrote {OUT}/exp2c_steering_results.json  "
...
print("\nALL EXP2C OUTPUT FILES:")
print(f"  {OUT}/exp2c_model.pt")
print(f"  {OUT}/exp2c_steering_results.json")
print(f"  {OUT}/exp2c_results.png")
