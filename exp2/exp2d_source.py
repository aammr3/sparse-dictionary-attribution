# %% [markdown]
# # Experiment 2d — Add a second, slot-native hit-rate metric (Phase F)
#
# **Foundation:** exp2d is a one-delta variant of `exp2/exp2c_source.py`.
# Training, model architecture, eval block, domain vocab lists, generation
# harness, collapse detector, target-slot selection, BOOST and SUPPRESS
# mechanism, theta/sigma-derived strengths, and the "distinctive corpus
# vocabulary" hit-rate metric are all unchanged from exp2c — same
# `n_slots=4096, k=16, reserved_per_domain=256`, same four domains
...
# `pred_dist`, same `target_slots[d]` selection (top-2-by-purity within
# each domain's reserved range), same `BOOST_STRENGTHS = [0, 0.5*theta,
# 1.0*theta, 1.5*theta, 2.0*theta, 4.0*theta]`, same `SUPPRESS_STRENGTHS =
# [0, 0.5*sigma, 1*sigma, 2*sigma, 4*sigma, 8*sigma]`, same auto-verdict
...
#
# **Outputs:**
# - `exp2d_model.pt` — the freshly trained model.
# - `exp2d_steering_results.json` — every metric per (domain,
...
# - `exp2d_results.png` — extends the exp1h-style plot with two extra
...
torch.save(model.state_dict(), f"{OUT}/exp2d_model.pt")
...
plt.tight_layout()
plt.savefig(f"{OUT}/exp2d_results.png", dpi=130)
plt.show()
...
with open(f"{OUT}/exp2d_summary.json", "w", encoding="utf-8") as f:
...
# %% [markdown]
# ## 11. Steering JSON dump — `exp2d_steering_results.json`
#
...
with open(f"{OUT}/exp2d_steering_results.json", "w", encoding="utf-8") as f:
    json.dump(steering_payload, f, indent=2, ensure_ascii=False)
print(f"\nwrote {OUT}/exp2d_steering_results.json  "
...
print("\nALL EXP2D OUTPUT FILES:")
print(f"  {OUT}/exp2d_model.pt")
print(f"  {OUT}/exp2d_steering_results.json")
print(f"  {OUT}/exp2d_results.png")
