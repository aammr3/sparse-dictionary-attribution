"""Generate exp2/exp2_kaggle.ipynb from exp2/exp2_source.py.

Splits the source on `# %%` markers:
  * `# %% [markdown]` blocks become markdown cells (the leading `# ` is stripped).
  * plain `# %%` blocks become code cells.

Output: a valid .ipynb (nbformat=4, nbformat_minor=5) with kernelspec python3
and language_info python, matching the structure of exp1/exp1h_kaggle.ipynb.

Self-contained: no project-external imports. Read the source, split, write JSON.
"""
import json
import os

HERE = os.path.dirname(os.path.abspath(__file__))
SRC = os.path.normpath(os.path.join(HERE, "exp2_source.py"))
DST = os.path.normpath(os.path.join(HERE, "exp2_kaggle.ipynb"))
...
def split_cells(text):
    ...
    for line in lines:
        if line.startswith("# %% [markdown]"):
            ...
            if cur_type is None:
                # any preamble before the first `# %%` marker: skip silently,
                # the exp1h notebook does the same (its source starts at `# %% [markdown]`).
                continue
            cur.append(line)
...
def to_source_array(lines):
    ...
    for i, ln in enumerate(lines):
        if i < len(lines) - 1:
            ...
            arr.append(ln)
    # Jupyter convention: ensure cells end with a trailing newline so the
    # next cell starts on a fresh line.
