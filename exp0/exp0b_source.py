...
#
# ---
# **Variant b note:** This is exp0 re-run with `k = 8` (the baseline used the
# double of this value). Triggered by exp0's baseline result coming back
# weak-positive (mean purity 0.766, gap +0.187) — testing whether forcing
# higher sparsity pushes slots into cleaner specialization. Everything else
# (domains, n_slots, steps, alpha schedule, losses, eval logic, output
# filenames) is identical to baseline exp0. This will run in a fresh Kaggle
# session, so no filename collisions with exp0's outputs.
...
class CFG:
    ...
    n_slots        = 1024    # 4x expansion
    k              = 8       # how many slots stay active per token (variant b: halved from baseline)
