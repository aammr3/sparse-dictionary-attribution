...
#
# ---
# **Variant c note:** This is exp0 re-run with `n_slots` doubled to 2048 and
# `k` restored to 16 (the better-performing value from the baseline exp0).
# Triggered by exp0b's result showing that lowering `k` hurt rather than
# helped: exp0b (the lower-`k` attempt) collapsed to mean purity 0.708 and
# gap +0.089, versus exp0's mean purity 0.766 and gap +0.187; exp0b's
# `final_lm_loss` also worsened to 3.982 vs. exp0's 3.262 because its
# `recon_loss` ballooned — the lower-`k` config gave the registry too little
# per-token capacity to reconstruct the residual stream accurately, and that
# reconstruction error poisoned both language quality and specialization. So:
# instead of less capacity via a lower `k`, try **more** capacity via a wider
# dictionary. Everything else (domains, steps, alpha schedule, losses, eval
# logic, output filenames) is identical to baseline exp0. This will run in a
# fresh Kaggle session, so no filename collisions with the prior runs.
